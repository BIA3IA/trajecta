...
export function PricingCtaButton({ label, href, className, variant }: PricingCtaButtonProps) {
  return (
    <Button asChild variant={variant} className={cn("group w-full overflow-hidden border-0 px-4 py-3", className)}>
      <a href={href}>
        ...
        <span className="pointer-events-none absolute inset-0 opacity-0 transition-opacity duration-200 group-hover:opacity-100">
          <span className="-skew-x-12 absolute inset-y-0 left-[-18%] w-24 animate-shimmer-sweep bg-white/10 blur-xl" />
        </span>
      </a>
    </Button>
  )
